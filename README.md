...
Codex ───────────────────────────────> OpenAI 官方模型
  ...
  │
  └─ PreCompact ────> 过滤后的增量对话 ────> TDAI MemoryCore
```
...
- `PreCompact` 在 Codex 自动或手动压缩上下文前调用 `capture_transcript`。
...
前三项测试分别验证 HTTPS 鉴权和 L0–L3 只读接口、隐私过滤、Memory MCP 握手及工具调用；第四项使用本机临时模拟服务验证 Knowledge MCP 的端点、隔离头和五个只读工具，不访问真实 TDAI，也不会写入测试记忆。
